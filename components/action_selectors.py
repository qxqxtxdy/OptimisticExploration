...
class NoiseGreedyActionSelector():
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        upper_bound = th.max(masked_q_values, dim=-1, keepdim=True)[0]
        lower_bound = th.min(masked_q_values, dim=-1, keepdim=True)[0]

        range = upper_bound - lower_bound

        noise = (th.rand_like(masked_q_values) * 2 - 1) * range * self.epsilon
        noise = noise.cuda()

        noise_q_valuse = masked_q_values + noise

        noise_q_valuse[avail_actions == 0.0] = -float("inf")  # should never be selected!

        picked_actions = noise_q_valuse.max(dim=2)[1]
        return picked_actions


REGISTRY["noise_greedy"] = NoiseGreedyActionSelector
